...
Public Class DelegadoDocumentoSolicitudStock
    Implements DelegadoDocumento

    Property solicitud As SolicitudDeStock
    Public Sub New(oSolicitud As SolicitudDeStock)
        Me.solicitud = oSolicitud
    End Sub
    ...
    Public Sub escribir(documento As iTextSharp.text.Document, escritor As iTextSharp.text.pdf.PdfWriter) Implements DelegadoDocumento.escribir
        ...
        headertable.AddCell("")
        Dim invoiceCell As PdfPCell = New PdfPCell(New Phrase("Comprobante de solicitud de ingreso de stock", titleFont))
        invoiceCell.HorizontalAlignment = 2
        invoiceCell.Border = Rectangle.NO_BORDER
        headertable.AddCell(invoiceCell)
        Dim noCell As PdfPCell = New PdfPCell(New Phrase("Número de solicitud :", bodyFont))
        noCell.HorizontalAlignment = 2
        noCell.Border = Rectangle.NO_BORDER
        headertable.AddCell(noCell)
        headertable.AddCell(New Phrase(Me.solicitud.nroPedido.ToString(), bodyFont))
        ...
        Dim billCell = New PdfPCell(New Phrase(" ", bodyFont))
        ...
        itemTable.SetWidths(New Integer() {10, 40, 10, 20})  ' then set the column's __relative__ widths
        ...
        Dim cell4 As PdfPCell = New PdfPCell(New Phrase("FECHA SOLICITUD", boldTableFont))
        ...
        Dim numberCell1 As PdfPCell = New PdfPCell(New Phrase(solicitud.producto.nombre, bodyFont))
        ...
        Dim numberCell2 As PdfPCell = New PdfPCell(New Phrase(solicitud.cantidad.ToString(), bodyFont))
        ...
        Dim numberCell3 As PdfPCell = New PdfPCell(New Phrase(solicitud.fecha.ToString(), bodyFont))
        ...
        Dim cai As PdfPCell = New PdfPCell(New Phrase("Sucursal Nro. : 1", boldTableFont))
