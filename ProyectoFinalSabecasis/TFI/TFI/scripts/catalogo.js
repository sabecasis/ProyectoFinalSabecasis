...
function imageNotFound(){
    this.onerror=null;this.src='../static/imagenes_de_producto/imageNotFound.gif';
}
...
function otenerDetalleProducto() {
    ...
    function onSuccess(result) {
        if (result != null) {
            ...
            document.getElementById('imagendescripcionprod').clientWidth = '200';
            document.getElementById('imagendescripcionprod').clientHeight = '200';
            ...
            document.getElementById('caracteristicas').innerHTML = result.caracteristicas;
            var divDescuentos = document.getElementById('divdescuentos');
            divDescuentos.innerHTML = '';
            if (result.descuentos.length > 0) {
                
                var header = document.createElement('h4');
                header.id = 'hdescuentosdescripcion';
                header.appendChild(document.createTextNode('Descuentos'));
                divDescuentos.appendChild(header);
                for (i = 0; i < result.descuentos.length; i++) {
                    var p = document.createElement('p');
                    p.appendChild(document.createTextNode(result.descuentos[i].descripcion));
                    divDescuentos.appendChild(p);
                }
...
function comparar() {
    if (nros != null) {
        var prodId = nros[0];
        var producto1 = { nroDeProducto: prodId, nombre: '' };
        PageMethods.obtenerProducto(producto1, onSuccess, onFailure);
        function onSuccess(result) {
            if (result != null) {
                document.getElementById('comparacion1').innerHTML = '<p><h2>'+ result.nombre+ '</h2></p>'+ result.caracteristicas;
            }
        }
        function onFailure(result) {
        ...
        var prodId2 = nros[1];
        var producto2 = { nroDeProducto: prodId2, nombre: '' };
        PageMethods.obtenerProducto(producto2, onSuccess2, onFailure2);
        function onSuccess2(result) {
            if (result != null) {
                document.getElementById('comparacion2').innerHTML = '<p><h2>'+ result.nombre +'</h2></p>' + result.caracteristicas;
            }
        }
        function onFailure2(result) {
            alert(result._message);
        }
...
function votarProducto() {
    function onSuccess(result) {}
    function onFailure(result) { }
    var valores = this.id.split('-');
    var nroProd = valores[0];
    var voto = valores[1];
    var valoracion = {
        producto: { nroDeProducto: nroProd },
        valoracion: voto
    };
    PageMethods.votarProducto(valoracion, onSuccess, onFailure);
}

function checkearComparacion() {
    var elementos = document.getElementsByName('comparaciones');
     nros = new Array();
    var cont = 0;
    for (i = 0; i < elementos.length; i++) {
        if (elementos[i].checked == true) {
            nros[cont] = elementos[i].id.substring(4);
            cont++;
        }
    }
    if (cont == 2) {
        document.getElementById('compararproductos').disabled = '';
    } else {
        document.getElementById('compararproductos').disabled = 'disabled';
    }
}
...
function obtenerCatalogo(idCatalogo, precio, conDescuento) {
    ...
    function onSuccess(result) {
        ...
        var divs = document.getElementsByName('lineaprod');
        while (divs.length > 0) {
        ...
        if (result != null && result.length > 0) {
            ...
            for (i = 0; i < result.length; i++) {
                if (i % 2 === 0) {
                    ...
                    fila.className = 'row';
                    fila.setAttribute('name','lineaprod');
                    contenido.appendChild(fila);
                }
                //El div que contiene el producto
                var prod = document.createElement('div');
                prod.className = 'col-sm-3 col-lg-3 col-md-3 thumbnail';
               ...
               //El div que contiene la imagen e info del producto
                ...
                //Si tiene descuento le ponemos la imagen con descuento
                if (result[i].descuentos.length > 0) {
                    var desc = document.createElement('img');
                    desc.className = 'img-descuento';
                    desc.src = '/static/descuento.png';
                ...
                var contenedorNombre = document.createElement('h6');
                ...
                //aca ponemos el checkbox que nos permite comparar productos
                var labelseleccion2 = document.createElement('span');
                labelseleccion2.appendChild(document.createTextNode(' Comparar '));
                labelseleccion2.name = 'spcomparar';
                labelseleccion2.className = 'datos-producto';
                var checkbox2 = document.createElement('INPUT');
                checkbox2.id = 'comp' + result[i].nroDeProducto;
                checkbox2.type = 'CHECKBOX';
                checkbox2.name = 'comparaciones';
                checkbox2.value = result[i].nroDeProducto;
                checkbox2.onchange = checkearComparacion;
                pseleccion.appendChild(labelseleccion2);
                pseleccion.appendChild(checkbox2);
                ...
                cantBox.value = 1;
                cantBox.onchange=checkearComparacion
                ...
                divestrellitas.id = 'rating-' + result[i].nroDeProducto;
                var estrellasvacias = 5 - Math.round(result[i].valoracion.valoracion);
                var estrellasllenas = Math.round(result[i].valoracion.valoracion);
                var nroEstrella = 1;
                if (estrellasllenas != 0) {
                    for (j = 0; j < estrellasllenas; j++) {
                        var estre1 = document.createElement('span');
                        estre1.className = 'glyphicon glyphicon-star';
                        var href1 = document.createElement('a');
                        href1.href = '#';
                        href1.onclick = votarProducto;
                        href1.id = result[i].nroDeProducto + '-' + nroEstrella;
                        href1.appendChild(estre1);
                        divestrellitas.appendChild(href1);
                        nroEstrella++;
                    }
                }
                if (estrellasvacias != 0) {
                    for (j = 0; j < estrellasvacias; j++) {
                        ...
                        href2.href = '#';
                        href2.onclick = votarProducto;
                        href2.id = result[i].nroDeProducto + '-' + nroEstrella;
                        href2.appendChild(estre2);
                        divestrellitas.appendChild(href2);
                        nroEstrella++;
                    }
                }
