     ...
     <script>
        ...
        function crearOpcionesDeCuotas() {
            ...
            for (i = 0; i < cuotas.length; i++) {
                ...
                op.value = i;
                op.text = cuotas[i].cantidadDeCuotas + '(' + cuotas[i].porcentajeDeRecargo + '%)';
        ...
        function validarTarjeta() {
            var informacionDePago = {
                metodo: { id: document.getElementById('metodo').value },
                anioVencimiento: document.getElementById('anio').value,
                mesVencimiento: document.getElementById('mes').value,
                nroDeTarjeta: document.getElementById('nro').value,
            ...
            function onSuccess(result) { }
            function onFailure(result) {
                document.getElementById('ContentPlaceHolder1_LblMensaje').innerText = result._message;
            }
            PageMethods.validarDatosDeTarjeta(informacionDePago, onSuccess, onFailure);
        }

        function aplicarNota(tieneRecargo, porcentajeRecargo) {
            function onSuccess(result) {
                document.getElementById('ContentPlaceHolder1_total').value = result;
                if (tieneRecargo != null && tieneRecargo == true) {
                    var recargo = (porcentajeRecargo * result) / 100;
                    document.getElementById('ContentPlaceHolder1_total').value = parseInt(result) + parseInt(recargo);
                } else {
                    calcularRecargo(document.getElementById('cantCuotas').value);
                }
            }
            function onFailure(result) {
                var t = result;
            }
            var nroNota = document.getElementById('notacredito').value;
            var total = document.getElementById('ContentPlaceHolder1_total').value;
            PageMethods.calcularMontoConNotaDeCredito(total.replace(",","."), nroNota, onSuccess, onFailure);
        }

        function obtenerCuotas(id) {
            function onSuccess(result) {
                cuotas = result.cuotas;
                crearOpcionesDeCuotas();
            }
            function onFailure(result) { }
            PageMethods.obtenerMetodoDePagoPorId(id, onSuccess, onFailure);
        }

       
        function calcularRecargo(id) {
            var porcentaje = cuotas[id].porcentajeDeRecargo;
            if (document.getElementById('notacredito').value != null
                && document.getElementById('notacredito').value != "") {
                document.getElementById('ContentPlaceHolder1_total').value = totalOriginal;
                aplicarNota(true, porcentaje);
            } else {
                
                var recargo = (porcentaje * parseFloat(totalOriginal.replace(",","."))) / 100;
                document.getElementById('ContentPlaceHolder1_total').value = parseInt(totalOriginal) + parseInt(recargo);
            }
            
        }

        function quitarNotaDeCredito() {
            document.getElementById('notacredito').value = "";
            calcularRecargo(document.getElementById('cantCuotas').value);
        }

        window.onload = function () {
            totalOriginal = document.getElementById('ContentPlaceHolder1_total').value;
            document.getElementById('ContentPlaceHolder1_totalOriginalAPagar').value = totalOriginal;
    ...
    <section id="contenedor">
        <input type="hidden" id="accion" name="accion" />
         <div id="contenido">
             ...
             <input type ="text" readonly="readonly" id="totalOriginalAPagar" name="totalOriginalAPagar" runat="server" style  ="visibility:hidden"/>
             <p />
             <span id="sptotal">Total  </span><input type ="text" readonly="readonly" id="total" runat="server" />
             <p />
             <span id="spmetodopago">Método de pago: </span><select id="metodo" name="metodo" onload="obtenerCuotas(this.value);" onchange="obtenerCuotas(this.value);validarTarjeta();" ></select>
            <p/> 
            <span id="spnumerotarjeta">Número de tarjeta: </span><input type="text" id="nro" name="nro" onchange="validarTarjeta()"/>
             <p/>
             <span id="sptitular">Titular de la tarjeta: </span><input type="text" id="titular" name="titular" onchange="validarTarjeta()" />
            <p/>
             <span id="spcvv">CVV: </span><input type="text" id="cvv" name="cvv" onchange="validarTarjeta()"/>
            <p/>
             <span id="spmes">Mes: </span><select id="mes" name="mes"  onchange="validarTarjeta()"></select>
            <p/>
             <span id="spanio">Año: </span><select id="anio" name="anio" onchange="validarTarjeta()"></select>
            <p/>
               <span id="spcantidadcuotas">Cantidad de cuotas: </span><select id="cantCuotas" name="cantCuotas" onchange="calcularRecargo(this.value)"></select>
             <p />
             <span id="Span1">Número de nota de crédito: </span><input type="text" id="notacredito" name="notacredito" />
             <input type="button" id="aplicarBtn" onclick="aplicarNota()" value="Aplicar nota de crédito"/><input type="button" value="deshacer" id="deshacerBtn" onclick="quitarNotaDeCredito()"/>
             <p/>
